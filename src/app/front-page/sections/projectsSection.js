            ...
            {projectsList.slice(0, 9).map((project, index) => (
              <div key={index} className="project-card bg-black-900">
                ...
                <div className="project-content p-4">
                  <h3 className="project-name text-white font-serif text-[1.25rem] ">{project.name}</h3>
                  ...
                  <div className="project-tags flex flex-wrap gap-2 mt-4">
                    {project.tags.map((tag, tagIndex) => (
                      <span key={tagIndex} className="project-tag bg-black-700/30 text-white/70 py-1 px-2">{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
